...
import { LayoutDashboard, User, Upload, Activity, History as HistoryIcon, FileText, Maximize, Minimize } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-white">
      ...
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 p-4 sticky top-0 z-50 shadow-sm overflow-hidden">
        <div className="max-w-7xl mx-auto flex items-center justify-between relative z-10">
          ...
          >
            ...
            <div className="flex flex-col">
              <h1 className="text-3xl font-display font-black tracking-tighter text-slate-900 leading-none group-hover:text-upi-red transition-colors title-glitch uppercase">
          ...
          <nav className="hidden xl:flex items-center gap-2">
            ...
            <NavButton active={currentPage === 'athlete-data'} onClick={() => setCurrentPage('athlete-data')} icon={<User className="w-4 h-4" />} label="Data Atlet" />
            <NavButton active={currentPage === 'assessment'} onClick={() => setCurrentPage('assessment')} icon={<Upload className="w-4 h-4" />} label="Upload Video" />
            {(testResults) && (
        ...
        <AnimatePresence mode="wait">
          ...
          {currentPage === 'athlete-data' && (
             <motion.div key="data" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <AthleteData savedData={athleteData} onNext={handleAthleteNext} />
             </motion.div>
          )}
             ...
             <motion.div key="assess" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                {!athleteData ? (
                    <div className="p-20 text-center space-y-6 premium-card">
                         ...
                         <p className="text-xl font-bold text-slate-400">Silakan isi Data Atlet terlebih dahulu.</p>
